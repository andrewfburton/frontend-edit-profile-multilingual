<?php
/*
Plugin Name: Frontend Edit Profile Multilingual
Version: 1.0.3
Description: Add edit profile to your post or page
Author: Abdul Ibad & AFB (for multilingual and some edits)
...
class FRONTEND_EDIT_PROFILE{
	...
	{
		$user_contact['skype'] = __( 'Skype', 'fep' ); 
		$user_contact['twitter'] = __( 'Twitter', 'fep' );
		$user_contact['yahoo'] = __( 'Yahoo', 'fep' );
		$user_contact['aim'] = __( 'AIM', 'fep' ); 
	...
	function process_login_form(){
		...
		if(empty($userlogin)){
			$this->wp_error = new WP_ERROR("invalid_username",__('<strong>ERROR</strong>: Empty username', 'fep'));
			return;
		}
		
		if(empty($userpass)){
			$this->wp_error = new WP_ERROR("incorrect_password",__('<strong>ERROR</strong>: Empty password', 'fep'));
			return;
		}
		...
		if ( is_wp_error($user) ){
			...
			switch(strtoupper($error_code)){
				...
				$this->wp_error = new WP_ERROR("invalid_username", __('<strong>ERROR</strong>: Invalid username', 'fep'));
				break;
				case "INCORRECT_PASSWORD":
				$this->wp_error = new WP_ERROR("incorret_password", __('<strong>ERROR</strong>: Incorrect password', 'fep'));
				break;
				default:
					$this->wp_error = $user;
				break;
